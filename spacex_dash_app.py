...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class', 
        title='Total Success Launches for the Selected Site')
        return fig
...
def update_bar_chart(entered_site,range_):
    filtered_df = spacex_df[spacex_df['Launch Site']==entered_site]
    low, high = range_
    mask = (spacex_df['Payload Mass (kg)'] > low) & (spacex_df['Payload Mass (kg)'] < high)
    mask_= (filtered_df['Payload Mass (kg)'] > low) & (filtered_df['Payload Mass (kg)'] < high)
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df[mask], x="Payload Mass (kg)", y="class", color='Booster Version Category',  
        title='Successful Launches vs. Payload Mass (kg)')
        return fig
    else:
        # return the outcomes piechart for a selected site
        fig = px.scatter(filtered_df[mask_], x="Payload Mass (kg)", y="class", color='Booster Version Category', 
        title='Successful Launches vs. Payload Mass (kg)')
        return fig
# Run the app
